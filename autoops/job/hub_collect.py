...
def get_hub_count():
        sql_qry_ip_user="select IpAddress,UpdUser from asset_appregioninfo_copy1"
        #db=db_operate()
           ...
           # print user
            for item in salt_ip_list:
                if ip==item['ip']:
                    ip=item['salt_name']
                    #print ip
                    str="su - %s -c 'sh ~/bin/gethubstat.sh'" %(user)
            loc=LocalClient()
...
def salt_ret_insert_db(list):      
    ...
    #list=[{'count': '0', 'ip': '172.16.49.81_uat-yhmk', 'regname': 'URMBAT', 'time': '09-03 10:28:16,671'}, {'count': '0', 'ip': '172.16.49.81_uat-yhmk', 'regname': 'URMOLN', 'time': '09-03 10:32:40,887'}]
        ...
        for item in list:
            sql_cmd="insert into asset_hubstat(ip,regname,time,count) values('%s','%s','%s','%s')" % (item['ip'],item['regname'],item['time'],item['count'])
           # print sql_cmd
            print sql_cmd
...
if __name__ == '__main__':
    ...
    scheduler.add_job(insert_hub_state, 'interval', seconds=40)
    scheduler.start()
