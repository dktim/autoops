...
function get_hub_state_at_start(){
    //var start=$('#d28').val();
    //var stop=$('#d29').val();
	...
	reg=$('#regname').text()
    jQuery.ajax({
        url: "/monitor/start_page_with_hub_state",
        data: {'reg':reg},
        type:"get",
        dataType:"json",
...
function query_hub_status(){
alert('use static dic~')
	var start=$('#d28').val();
	var stop=$('#d29').val();
//	var reg=$("#province  option:selected").text();
   ...
   //alert(reg)
	...
	jQuery.ajax({
	    url: "/monitor/get_hub_ajax_state",
	    data: {"start" : start, "stop" : stop,"reg":reg,"ip":ip},
	    type:"get",
	    dataType:"json",
	 	success: function(data){
   if (data.count.length==0 || data.time.length==0){
alert('No hub data matched!')
      			  var time_list=[]
                   	 var count = []
