...
def mylogin(request):
    ...
    if request.POST:
        ...
        if user is not None:
            ...
            msg.append("Password error or user not exists")
...
def add_user(request):
    ...
    if request.method=="POST":
	    ...
	    _password=make_password(_password)
            sql="insert into AUTH_USER(ID,username,password,email,is_active,is_superuser,is_staff) values(%d,'%s','%s','%s',%d,%d,%d)"%(id,_username,_password,_email,1,_superuser,1)
            print sql
